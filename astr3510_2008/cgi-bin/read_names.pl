...
my $prevpage = "../observing.htm";


#Get and untaint the data
my $group = $cgiForm->param('name');
my $month = &untaint($cgiForm->param('month'));
my $day   = &untaint($cgiForm->param('day'));
my $time  = $cgiForm->param('time');
my $class = $cgiForm->param('function');
my $other = $cgiForm->param('other');
...
print " charset=utf-8\"/>\n";
print "  <title>ASTR 3510: Sign-Up Submission</title>\n";
print "\n";
...
if ((($day eq "31") && (($month eq "February") || ($month eq "April"))) || ( (($day eq "30") && ($month eq "February")))) {
...
#    }
  ...
  if($reserved) {
    ...
    my $funcn    = ($class eq "ASTR 3510") ? "3510" :
                                             "Other";
    if ($funcn eq "Other") {
...
#	print "<br> Was there some problem? $#groups";
    ...
    if ($#groups == -1) {
	  ...
	  if ($day < $days[$i]) {
	    ...
	    if ($itime < $itimes[$i]) {
          ...
          print OUTFILE "$months[$i] $days[$i]\t$times[$i]\t$funcns[$i]\t$groups[$i]\t$others[$i]\n";
          $inserted = 1;
        } elsif ((!$inserted) && ($months[$i] eq "" || $months[$i] eq "#" || $months[$i] =~ /^\s+$/)) {
          print OUTFILE "$month $day\t$time\t$funcn\t$group\t$other\n";
          print OUTFILE "$months[$i] $days[$i]\t$times[$i]\t$funcns[$i]\t$groups[$i]\t$others[$i]\n";
          $inserted = 1;
        } else {
    ...
    if ($notify) {
      ...
      print SENDMAIL "Subject: $group is observing at $time on $month $day\n";
      print SENDMAIL "Content-type: text/plain\n\n";
      print SENDMAIL "$group has signed up to use the 24-inch.\n\n";
      print SENDMAIL "Date:  $month $day\n";
      print SENDMAIL "Time:  $time\n";
      print SENDMAIL "Group: $group\n";
      print SENDMAIL "Class: $class\n";
      print SENDMAIL "Task: $other\n";
      close(SENDMAIL);
    }
  }
}
